...
public class CachingServiceIT {
    ...
    public void englishLocaleWelcomeMessage_caches(){
        ...
        Mockito.when(mockRetryService.getWelcomeMessageAndHandleTimeout("en_us")).thenReturn(
                Mono.defer(()-> Mono.just(new WelcomeMessage("count"+counter.incrementAndGet()))));
...
@Test
    ...
    Mockito.when(mockRetryService.getWelcomeMessageAndHandleTimeout(anyString())).thenAnswer(new Answer<Mono<WelcomeMessage>>() {
        @Override
        public Mono<WelcomeMessage> answer(InvocationOnMock invoaction) throws Throwable{

            String locale_arg = invoaction.getArgument(0);
            return Mono.defer(()->{
...
for (int i=0;i<5;i++){
    StepVerifier.create(cachingService.getCachedWelcomeMono("ru"))
            .expectNextMatches(welcomeMessage -> "local ru" . equals(welcomeMessage.getMessage())).verifyComplete();
}
