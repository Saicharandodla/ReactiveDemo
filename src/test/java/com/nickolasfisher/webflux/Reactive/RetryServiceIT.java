...
public class RetryServiceIT {
    ...
    public RetryServiceIT(ClientAndServer clientAndServer){
        ...
        HttpClient httpClient = HttpClient.create().tcpConfiguration(tcpClient ->
                tcpClient.option(ChannelOption.CONNECT_TIMEOUT_MILLIS,WEBCLIENT_TIMEOUT).doOnConnected(connection -> connection.addHandlerLast(
    ...
    public void retryOnTimeout(){
        ...
        HttpRequest expectedRequest = request().withPath("/locale/en_us/message").withMethod(HttpMethod.GET.name());

        this.clientAndServer.when(expectedRequest).respond(httpRequest -> {
      if(atomicInteger.incrementAndGet()<2){
          Thread.sleep(WEBCLIENT_TIMEOUT+10);
        }
      return HttpResponse.response().withBody("{\"message\":\"hello\"}").withContentType(MediaType.APPLICATION_JSON);
        });

        StepVerifier.create(retryService.getWelcomeMessageAndHandleTimeout("en_us")).expectNextMatches(welcomeMessage -> "hello".equals(welcomeMessage.getMessage())).verifyComplete();
