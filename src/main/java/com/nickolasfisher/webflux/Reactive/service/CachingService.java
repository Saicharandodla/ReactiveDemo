...
public class CachingService {
    ...
    private final Cache<String, WelcomeMessage> WELCOME_MESSAGE_CACHE = Caffeine.newBuilder().
            expireAfterWrite(Duration.ofMinutes(5)).maximumSize(1_000).build();
...
this.cachedEnglishWelcomeMono = this.retryService.getWelcomeMessageAndHandleTimeout("en_us")
    ...
    public Mono<WelcomeMessage> getCachedEnglishWelcomeMono (){
        return cachedEnglishWelcomeMono;
    }
    public Mono<WelcomeMessage> getCachedWelcomeMono(String locale) {
        ...
        return message.map(Mono::just).orElseGet(()->this.retryService.getWelcomeMessageAndHandleTimeout(locale).doOnNext(welcomeMessage -> WELCOME_MESSAGE_CACHE.put(locale,welcomeMessage)));
    }
